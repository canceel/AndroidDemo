...
/**
 * 自定义VIEW实现倒计时圆盘
 * Created by CANC on 2016-10-11.
 */
public class CountdownBoard extends View {
    ...
    public CountdownBoard(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制刻度
        paintScale(canvas, mScaleColor, time);
        //绘制中心文字
        paintText(canvas, mTextColor, time);
        //重置
        canvas.restore();
        //刷新
        if (time > 0) {
    ...
    //绘制中心文字
    private void paintText(Canvas canvas, int color, int time) {
        canvas.save();
        //绘制文字
        ...
        canvas.drawText(time + "S", 0, 20, mPaint);
    }

    //绘制刻度
    private void paintScale(Canvas canvas, int color, int t) {
